...
module Control.CPFD.Fuzzy.Solver2
       (
       -- * Monads
         FD, FDS, FDState(..), cntVarGet, cntVarSet, cntConsDeg
       , runFD, runFD'
       -- * Data Types
       , NTLike(..), GNTLike(..), HasLift(..), ToList(..)
       -- * Variables and Domains
       , Grade, RGrade, Domain, FDValue, Var, NVar
       , new, newL, newN, newNL, newT, newTL, newGL
       -- * Constraint Store
       , add1, add2, addN
       -- * Labelling and Optimization
       , Stream, LabelCtx
       , labelAll, labelAllT
       , label, labelT
       , optimize, optimizeT
...
-- | Same as 'new' except to take a Traversable containing lists as domains.
newGL :: (GNTLike t [] (Var s)) => t [] -> FDS s (t (Var s))
newGL = gntA
...
testNewOpt = do
  (TupleV v) <- newGL (TupleV p1)
  add2 "parity" parity (v^._1) (v^._2)
  labelAll (TupleV v)

{-|
> runFD testNewOpt2 ^. _1 :: [([Int], RGrade)]
[([1,1],7 % 10),([1,2],3 % 10),([1,3],3 % 10),([1,4],3 % 10),([1,5],3 % 10),([2,1],3 % 10),([2,2],7 % 10),([2,3],3 % 10),([2,4],3 % 10),([2,5],3 % 10),([3,1],3 % 10),([3,2],3 % 10),([3,3],7 % 10),([3,4],3 % 10),([3,5],3 % 10),([4,1],3 % 10),([4,2],3 % 10),([4,3],3 % 10),([4,4],7 % 10),([4,5],3 % 10),([5,1],3 % 10),([5,2],3 % 10),([5,3],3 % 10),([5,4],3 % 10),([5,5],7 % 10)]
-}
-- Elements stored in Traversable
testNewOpt2 :: Stream m => FDS s (m ([Int], RGrade))
testNewOpt2 = do
  let d = [1, 2, 3, 4, 5]
  -- (TraversableV v) <- newGL (TraversableV [d,d])
  -- v <- traverse newL [d, d]
  -- v <- newNL 2 d
  v <- newTL [d, d]
